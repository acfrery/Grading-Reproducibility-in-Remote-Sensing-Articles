...
%DIF DEL GradingReproducibilityRS_R0.tex   Fri May  1 14:13:55 2020
%DIF ADD GradingReproducibilityRS_R1.tex   Fri Aug 14 09:06:44 2020

\usepackage{texnames}
\usepackage{graphicx}
%DIF 5c5-9
%DIF < \graphicspath{{../../../Figures/PDF/}{../../../Images/PNG/}}
%DIF -------
\graphicspath{ %DIF > 
{../../../Figures/PDF/} %DIF > 
{../../../Figures/PNG/} %DIF > 
...
\author{Alejandro~C.~Frery,~\IEEEmembership{Senior Member,~IEEE,}
	...
	\DIFdelbegin %DIFDELCMD < \thanks{A.\ C.\ Frery and A.\ C.\ Medeiros are with the \textit{Laborat\'orio de Computa\c c\~ao Cient\'ifica e An\'alise Num\'erica} -- LaCCAN, Universidade Federal de Alagoas, Macei\'o, Brazil.
%DIFDELCMD < 		A.\ C.\ Frery is also with the Key Lab of Intelligent Perception and Image Understanding of the Ministry of Education, Xidian University, Xi'an, China. (email: acfrery@laccan.ufal.br)}%%%
\DIFdelend \DIFaddbegin \thanks{A.\ C.\ Frery is with the School of Mathematics and Statistics, Victoria University of Wellington, 6140 New Zealand, and with the Key Lab of Intelligent Perception and Image Understanding of the Ministry of Education, Xidian University, Xi'an, 710126 China (e-mail: alejandro.frery@vuw.ac.nz).}\DIFaddend % <-this % stops a space
	\DIFdelbegin %DIFDELCMD < \thanks{Luis Gomez is with the Universidad de Las Palmas de Gran Canaria, Spain}%%%
\DIFdelend \DIFaddbegin \thanks{Antonio~C.~Medeiros is with the with the \textit{Laborat\'orio de Computa\c c\~ao Cient\'ifica e An\'alise Num\'erica} -- LaCCAN, Universidade Federal de Alagoas, Macei\'o, 57072-900 Brazil (e-mail: antoniomedeiros@laccan.ufal.br).}\DIFaddend % <-this % stops a space
	\DIFaddbegin \thanks{Luis Gomez is with the Universidad de Las Palmas de Gran Canaria, 35017 Spain (e-mail: luis.gomez@ulpgc.es).}%DIF >  <-this % stops a space
...
Most \DIFdelbegin \DIFdel{remote sensing }\DIFdelend \DIFaddbegin \DIFadd{Remote Sensing }\DIFaddend papers support the evidence of the proposed research by computational codes of medium or high complexity. 
...
We do not think so. 
\DIFaddbegin \DIFadd{Balz and Rocca~\mbox{%DIFAUXCMD
\cite{ReproducibilityandReplicabilityinSARRemoteSensing} }\hspace{0pt}%DIFAUXCMD
conducted an online survey on focused on experienced scientists, including editors
...
With \num{250} responses, of which only one did not complete the survey, they concluded that \mbox{%DIFAUXCMD
\SI{75}{\percent} }\hspace{0pt}%DIFAUXCMD
of the respondents encountered problems when trying to reproduce or replicate results from works published in some of the major Journals of the area.
}

\DIFaddend Many authors continuously strive to disseminate their research findings in such a way that any user will be able to validate them at a later stage. 
...
\begin{figure}[hbt]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \subcaptionbox{Open Data}{\includegraphics[width=.2\columnwidth]{OpenData}}
%DIFDELCMD < 	\subcaptionbox{Open Materials}{\includegraphics[width=.2\columnwidth]{OpenMaterials}}
%DIFDELCMD < 	\subcaptionbox{Preregistered}{\includegraphics[width=.2\columnwidth]{Preregistered}}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \subcaptionbox{Open Data.}{\includegraphics[width=.2\columnwidth]{OpenData}}
...
\begin{figure}[hbt]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \subcaptionbox{Functional artifacts\label{Fig:Functional}}{\includegraphics[width=.19\columnwidth]{artifacts_evaluated_functional}}
%DIFDELCMD < 	\subcaptionbox{Reusable artifacts\label{Fig:Reusable}}{\includegraphics[width=.19\columnwidth]{artifacts_evaluated_reusable}}
%DIFDELCMD < 	\subcaptionbox{Available artifacts\label{Fig:Available}}{\includegraphics[width=.19\columnwidth]{artifacts_available}}
%DIFDELCMD < 	\subcaptionbox{Reproducible results\label{Fig:Reproducible}}{\includegraphics[width=.19\columnwidth]{results_reproduced}}
%DIFDELCMD < 	\subcaptionbox{Replicable results\label{Fig:Replicable}}{\includegraphics[width=.19\columnwidth]{results_replicated}}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \subcaptionbox{Functional artifacts.\label{Fig:Functional}}{\includegraphics[width=.19\columnwidth]{artifacts_evaluated_functional}}
	...
	\DIFaddendFL \caption{ACM five-level badging system\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }\label{fig:ACM-Badging}
\end{figure}
...
\DIFaddbegin \DIFadd{Its contents are reviewed, and accepted contributions receive a DOI (Digital Object Identifier).
}\DIFaddend As such, RSCD is a significant player in the Remote Sensing reproducibility and replicability game.
...
The first requisite for being considered a \DIFdelbegin \DIFdel{RSRR }\DIFdelend \DIFaddbegin \DIFadd{Remote Sensing Reproducible Research (RSRR) paper }\DIFaddend is associating the manuscript to a universally accessible and informative web page containing:
\begin{enumerate}
...
Although Open data are not necessarily free data, the authors must at least provide \DIFdelbegin \DIFdel{free samples}\DIFdelend \DIFaddbegin \DIFadd{a minimum of free samples, and the tools to read and export them to freely available computational platforms}\DIFaddend .
...
\DIFaddbegin \subsubsection{\DIFadd{Additional artifacts}}
\DIFadd{Current collaborative research usually produces artifacts that may contribute to the replicability of the research.
...
\DIFaddend \subsection{Implementation}\label{Sec:Implementation}
...
\DIFaddbegin \DIFadd{The role of such committee is solely verifying the reproducibility of the article and, thus, the expertise of their members might also be from outside the Remote Sensing community.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Each group is identified by a color: ``Why?'' in purple,
``What?'' in yellow,
``Who?'' in brown,
``How?'' in green,
``When and how much?'' in pink.
}\DIFaddend This board can be copied and assigned to a team, whose leader can assign tasks (cards), with deadlines, reminders, and several other utilities.
...
\begin{figure}[hbt]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{ScientificCanvas}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{Canvas-Trello-Modelo4}
	\DIFaddendFL \caption{A Trello implementation of a Scientific Canvas\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }\label{Fig:ScientificCanvas}
\end{figure}
...
Any reproducible approach must pay special attention to what this so efficient machine learning approach is capable of getting from data (especially from \DIFdelbegin \DIFdel{the }\DIFdelend Big Data)\DIFdelbegin \DIFdel{.
Isdahl and Gundersen~\mbox{%DIFAUXCMD
\cite{OutoftheBoxReproducibilityaSurveyofMachineLearningPlatforms}}\hspace{0pt}%DIFAUXCMD
...
\DIFadd{The opinions expressed in this presentation are solely those of the presenter and not necessarily those of IEEE, IEEE Geoscience and Remote Sensing Society and its affiliates.
}

\DIFadd{All product and company names are trademarks or registered trademarks of their respective holders. The use of them does not imply any affiliation with or endorsement by them. 
}
...
\DIFaddbegin \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{ACFrery.jpg}}]{Alejandro C.\ Frery}\DIFadd{(S'92--SM'03)
...
He is currently an Associate Editor of the IEEE Geoscience and Remote Sensing Letters, and belongs to the Editorial Board of IPOL (Image Processing on Line, }\url{www.ipol.im}\DIFadd{). 
He researches at CTIM (Image Technology Center (}\url{www.ctim.es}\DIFadd{) and is an IEEE Senior Member.
}\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{AntonioMedeiros.jpg}}]{Antonio C.\ Medeiros} \DIFadd{graduated in Meteorology from the Universidade Federal de Alagoas (2016).
...
He is the Project Manager of }\textit{\DIFadd{Laborat\'orio de Computa\'c\~ao Cient\'ifica e An\'alise Num\'erica -- LaCCAN}} \DIFadd{of the latter University.
